...
export const EditProfile = () => {
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      uploadData.append('userId', store.currentUser.id)
      ...
      setImagePreview(formData.profile_image || "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_640.png")
    } finally {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`${host}api/users/${store.currentUser.id}`, {
        method: 'PUT',
        headers: {
