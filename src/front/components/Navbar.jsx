    ...
    <nav className="relative text-xl text-white">
      <ul className="relative flex items-center bg-black/90 py-3 px-3 md:px-5 lg:px-16">
        <li className="md:hidden" onClick={handleMenu}>
          {
            !menuIsActive ?
              <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" className="icon icon-tabler icons-tabler-outline icon-tabler-menu"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M4 8l16 0" /><path d="M4 16l16 0" /></svg>
              :
        ...
        <div className="hidden md:flex md:items-center md:gap-8 md:ml-auto">
          ...
          {isLogged && store.currentUser.is_admin && (
            <li>
              <Link to="/reviews" className="hidden md:block hover:bg-green-450 hover:rounded py-1 px-2 transition">Reseñas</Link>
            </li>
          )}
        </div>

        <li className="ml-auto md:ml-14 flex items-center gap-2" onClick={() => setMenuIsActive(false)}>
          {isLogged && store.currentUser?.is_admin && (
            <span className="hidden md:inline-block px-2 py-1 text-xs font-semibold rounded-full bg-green-350 text-white">
              admin
            </span>
          )}
          {isLogged && store.currentUser?.is_admin && (
            <span className="md:hidden ml-2 px-2 py-1 text-xs font-semibold rounded-full bg-green-350 text-white">
              admin
            </span>
          )}

          <Link to="/login" onClick={handleLogout} className="md:hover:scale-105">
            {
              isLogged ?
          ...
          <div className="bg-black w-full h-screen absolute z-50 text-3xl font-bold md:hidden">
            <ul className="flex flex-col gap-4 w-full p-4">
              ...
              )}
              {
                isLogged && store.currentUser.is_admin && (
                  <li className="group">
                    <Link to="/reviews" className="relative rounded-xl overflow-hidden bg-gradient-to-br from-brown-550 to-brown-150 border border-green-250 flex items-center hover:scale-105 hover:contrast-125 transition px-4 py-3 gap-4" onClick={() => setMenuIsActive(false)}>
                      <svg className="group-hover:scale-150 group-hover:-rotate-12 transition" xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="0.75" strokeLinecap="round" strokeLinejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M3 7a2 2 0 0 1 2 -2h14a2 2 0 0 1 2 2v10a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2v-10z" /><path d="M3 7l9 6l9 -6" /></svg>
                      <span>Reseñas</span>
                    </Link>
                  </li>
